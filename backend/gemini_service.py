...
You MUST actively consider all five bins below before deciding. Do not default to
"wet" or "dry" just because an item isn't food or paper — check it against
"recyclable", "hazardous", and "ewaste" first.
...
def _apply_keyword_override(result: dict) -> dict:
    text = f"{result.get('item', '')} {result.get('material', '')}".lower()

    if any(kw in text for kw in EWASTE_KEYWORDS):
        result["bin"] = "ewaste"
    elif any(kw in text for kw in HAZARDOUS_KEYWORDS):
        result["bin"] = "hazardous"
    elif result.get("bin") in ("wet", "dry") and any(kw in text for kw in RECYCLABLE_KEYWORDS):
        result["bin"] = "recyclable"
...
def analyze_waste(image_bytes: bytes, mime_type: str):
    ...
    result = _apply_keyword_override(result)
